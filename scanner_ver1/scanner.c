...
Token *readIdentKeyword(void)    //return Type là Ident hay Keyword
{
  ...
  while ((currentChar != EOF) && 
	 ((charCodes[currentChar] == CHAR_LETTER) || (charCodes[currentChar] == CHAR_DIGIT))) {   //ta đọc nếu gặp toàn kí tự letter hoặc number
    if (count < MAX_IDENT_LEN) token->string[count++] = toupper((char)currentChar);        //trong khuôn khổ  mong muốn nạp kí tự vào token.string
    readChar();
  }
...
Token *readNumber(void)
{
  ...
  while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT)) {
    if (count <= MAX_NUMBER) token->string[count++] = (char)currentChar;
    readChar();
  }
  if (count > MAX_NUMBER) {
...
Token* getToken(void) {
  ...
  case CHAR_SLASH:                                 // ki tu '/'
    ...
    readChar(); 
    return token;
  case CHAR_SPACE: skipBlank(); return getToken();  // gap dau cach thi skip,de quy de thuc hien tiep
  case CHAR_LETTER: return readIdentKeyword();      //gặp kí tự alpha hoặc gạch dưới _
  case CHAR_DIGIT: return readNumber();             //gap number
  
  case CHAR_LT:                                     //<
    ln = lineNo;
    cn = colNo;
    readChar();
    if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {    // doc them '=' tuc la token '<='
      readChar();
  ...
  case CHAR_COLON:                                  //':'
    ...
    cn = colNo;
    readChar();
    if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) { //phep gan ':='
      readChar();
      return makeToken(SB_ASSIGN, ln, cn);
    } else return makeToken(SB_COLON, ln, cn);
  case CHAR_SINGLEQUOTE: return readConstChar();    //gap dau mo ngoac nhay thi dung ham readConstChar doc hang ki tu
  case CHAR_LPAR:                                   //gap mo ngoac don '('
    ...
    switch (charCodes[currentChar]) {               //mot so truong hop nhu '(.' tra ve SB_LSEL,         
    case CHAR_PERIOD:
      readChar();
